var page    =   require('webpage').create(),
    url     =   'http://127.0.0.1:8080/banner-static-one-tag.html',
    ...
    if(response.url.match(/\/js\/response\-banner\.js/) && !received){
        ...
        setTimeout(function(){
            ...
            var src = page.evaluate(function(){
                return document.querySelector(".mobfox_iframe").parentNode.nextSibling.src;
            });

            var nextElmClass = page.evaluate(function(){
                return document.querySelector(".mobfox_iframe").parentNode.nextElementSibling.nextElementSibling.className;
            });

            test.ok(id.match(/^mobfox_test$/));
            test.equal(width,"320");
            test.equal(height,"50");
            test.ok(src.match(/ad\.js/));
            test.equal(nextElmClass,"green");
